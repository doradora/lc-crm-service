...
class Project(models.Model):
    ...
    manager = models.ForeignKey(User, on_delete=models.SET_NULL, null=True)  # 案件負責人
    drawing = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='drawing_user')  # 繪圖
    drawing_other = models.CharField(max_length=255, blank=True, null=True)  # 其他繪圖人員名字
    ...
    category = models.ForeignKey(Category, on_delete=models.SET_NULL, null=True)  # 案件類別
    ...
    invoice_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)  # 請款金額
    ...
    class Meta:
        unique_together = ('year', 'category', 'project_number')  # 年份+案件類別+編號不可重複
    ...
    def save(self, *args, **kwargs):
        if not self.project_number:
            last_project = Project.objects.filter(year=self.year).order_by('-project_number').first()
            if last_project:
